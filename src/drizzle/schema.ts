...
// pg-core becuase using postgres
...
export const ProductTable = pgTable(
  ...
  (t) => [index("clerk_user_id_index").on(t.clerkUserId)]
);
...
export const productRelations = relations(ProductTable, ({ one, many }) => {
  ...
  return {
    ...
    countryGroupDisocunts: many(CountryGroupDiscountTable),
  };
});

// onDelete: "cascade" means when the product with that ID is delete, this
  ...
  ({ one }) => {
    return {
      product: one(ProductTable, {
        fields: [ProductCustomizationTable.productId], // foreign key
        references: [ProductTable.id], // key on other table
      }),
    };
  }
);
...
export const CountryGroupDiscountTable = pgTable(
  ...
  (t) => ({
    pk: primaryKey({ columns: [t.countryGroupId, t.productId] }),
  })
);
...
export const UserSubscriptionTable = pgTable(
  "user_subscriptions",
  {
    ...
    stripeSubscriptiuonItemId: text("stripe_subscription_item_id"),
  ...
  (table) => {
    // indexes
    return [
      index("user_subscription_table.clerk_user_id_index").on(
        table.clerkUserId
      ),
      index("user_subscription_table.strpe_customer_id_index").on(
